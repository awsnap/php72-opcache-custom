...
static ZEND_FUNCTION(opcache_reset)
{
	...
	//int key_length;
	//char *filename = "somefile.php";

	//key = accel_make_persistent_key(ZSTR_VAL(filename), ZSTR_LEN(filename), &key_length);
	//if (key != NULL) {
	//zend_persistent_script *persistent_script = zend_accel_hash_str_find(&ZCSG(hash), key, key_length);
	//if (persistent_script && !persistent_s
...
static ZEND_FUNCTION(opcache_invalidate)
{
	...
	zend_accel_error(ACCEL_LOG_DEBUG, "Let the nuking begin...");
	if (zend_accel_hash_unlink(&ZCSG(hash), script_name, script_name_len) == SUCCESS)
	{
		zend_accel_error(ACCEL_LOG_DEBUG, "It's really reaching that place.");
